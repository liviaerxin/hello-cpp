 ...
 * OSX:(use X11 provided OpenGL rather than framework OpenGL)
clang++ XOpenGLModernWindow.cpp -o XOpenGLModernWindow \
-I/opt/X11/include \
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < fbcount; ++i) {
    ...
    if (vi != 0) {
      ...
      // std::cout << "  Matching fbconfig " << i << ", SAMPLE_BUFFERS = " <<
  ...
  window = XCreateWindow(display, RootWindow(display, screenId), 0, 0, 320, 200,
